(function (root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    define('simple-gallery', ["jquery","simple-module","simple-util"], function (a0,b1,c2) {
      return (root['gallery'] = factory(a0,b1,c2));
    });
    ...
    root.simple['gallery'] = factory(jQuery,SimpleModule,simple.util);
  }
...
Gallery = (function(superClass) {
  ...
  Gallery.prototype._render = function() {
    ...
    return setTimeout(((function(_this) {
      return function() {
        ...
        return simple.util.preloadImages(_this.curOriginSrc, function(originImg) {
  ...
  Gallery.prototype._bind = function() {
    this.galleryWrapper.on('click.gallery', (function(_this) {
      return function(e) {
  ...
  Gallery.prototype._renderImage = function() {
    var originSize, showZoom, stageSize, thumbImg, win;
    if (!this.galleryEl) {
      return;
    }
    win = $(window);
    ...
    return this.galleryEl.addClass('loading').find('.zoom-in').toggle(showZoom);
  };
  ...
  Gallery.prototype._onGalleryThumbClick = function(e) {
    ...
    this._renderImage();
    simple.util.preloadImages(this.curOriginSrc, (function(_this) {
      return function(img) {
        if (img.src.indexOf(_this.curOriginSrc) !== -1) {
          _this.galleryEl.removeClass('loading');
          return _this.imgEl.attr('src', img.src);
        }
      };
    })(this));
    return false;
  ...
  Gallery.prototype._rotate = function() {
    ...
    if (isOrthogonal) {
      if (simple.util.browser.firefox && imgSize.height < imgSize.width) {
  ...
  Gallery.prototype._preloadOthers = function() {
    ...
    return simple.util.preloadImages(othersEl);
  };
  ...
  Gallery.prototype._renderNatural = function() {
    var left, top;
    this.galleryWrapper.find('.natural-image').remove();
    this.imgEl.clone().css({
      width: this.curOriginSize.width,
      height: this.curOriginSize.height
    }).wrap('<div class="natural-image"></div>').parent().appendTo(this.galleryWrapper);
    left = top = 'auto';
    if (this.curOriginSize.width > window.innerWidth) {
      left = 0;
    }
    if (this.curOriginSize.height > window.innerHeight) {
      top = 0;
    }
    return this.galleryWrapper.find('.natural-image img').css('margin', top + " " + left);
  };

  Gallery.prototype.destroy = function() {
    $('html').removeClass('simple-gallery-active');
    this._unbind();
    this.galleryWrapper.removeClass('modal').find('.natural-image').remove();
    ...
    this.galleryEl.css(this.curThumbSize);
    return this.galleryEl.one(simple.util.transitionEnd(), (function(_this) {
      return function(e) {
